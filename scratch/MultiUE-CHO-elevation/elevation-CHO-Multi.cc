...
{
    double servingElev = serving.elevation;
    double targetElev  = neighbor.elevation;

    return targetElev > servingElev - reportingOffset;
}
...
MeasurementReport* UE_EvaluateAndReport(int ueId, double simTime)
{
    ...
    for (auto& meas : measurements)
    {
        ...
        if (meas.elevation > bestElev)
        {
            bestElev = meas.elevation;
            bestNeighbor = &meas;
        }
    }
...
void CHO_TriggerProcedure(int ueId, const MeasurementReport* report)
{
    ...
    double sourceElev = report->servingCell.elevation;
    double targetElev = report->neighborCells.empty() ? 0.0 : report->neighborCells[0].elevation;
    ...
    std::cout << "  Source: " << choState.sourceCell 
              ...
              << "Elev=" << std::setprecision(1) << sourceElev << " deg)" << std::endl;
    std::cout << "  Target: " << choState.targetCell 
              << " (SINR=" << std::setprecision(2) << choState.targetSnrAtReport << " dB, "
              << "Elev=" << std::setprecision(1) << targetElev << " deg)" << std::endl;
...
void CHO_CheckExecutionCondition(int ueId, double currentTime)
{
    ...
    double servingElev = servingIt->second.second.elevation;
    double targetElev  = targetIt->second.second.elevation;
    double elevImprovement = targetElev - servingElev; 

    // Phase-2 strict threshold: Target SNR must be greater than Serving SNR + Hysteresis (e.g., 3dB)
    bool conditionMet = (elevImprovement >= g_choExecutionOffset) &&
                        (servingElev >= 0.0);  // serving satellite still visible
...
std::string SelectBestSatellite(int ueId, const std::string& currentSat)
{
    ...
    for (const auto& pair : ue.channelCache)
    {
        ...
        if (metrics.elevation > bestElev)
        {
            bestElev = metrics.elevation;
            bestSat = satName;
            std::cout << "    → New best candidate: " << bestSat 
                    << " (Elevation=" << std::setprecision(1) << bestElev << " deg)" << std::endl;
        }
    }
    
    // Apply elevation hysteresis: only switch if new satellite is significantly better
    if (bestSat != currentSat)
    {   
        if (bestElev - currentElev < g_choExecutionOffset)
        {
...
main(int argc, char* argv[])
{
    ...
    mkdir("result", 0755);
    std::string outputDir = "result/elevation-cho";

...
double g_reportingOffset = 120.0;
double g_minSNR = 0.0;
double g_handoverHysteresis = 3.0;
double g_timeToTrigger = 1.0;
double g_maxDistance = 1800.0;
double g_minServingTime = 360.0;
double g_choExecutionOffset = 120.0;
...
const double TRAJECTORY_GAP_THRESHOLD = 90.0;  // seconds; larger gaps mean a new visible pass
...
bool LoadTrajectory(const std::string& csvFile, const std::string& satName)
{
    ...
    satState.max_time = std::max(1.0, trajectory.back().time);
    satState.remaining_time = satState.max_time;
...
void RefreshSatelliteTiming(double simTime)
{
    for (auto& satPair : g_satellites)
    {
        auto& sat = satPair.second;
        if (sat.trajectory.empty())
        {
            sat.max_time = 1.0;
            sat.remaining_time = 0.0;
            continue;
        }

        if (sat.max_time <= 0.0)
        {
            sat.max_time = std::max(1.0, sat.trajectory.back().time);
        }

        sat.remaining_time = std::max(0.0, sat.trajectory.back().time - simTime);
    }
}

bool FindTrajectoryBracket(const std::vector<TrajectoryPoint>& trajectory, double simTime, size_t& leftIndex)
{
    if (trajectory.empty())
    {
        return false;
    ...
    if (simTime < trajectory.front().time || simTime > trajectory.back().time)
    {
        return false;
    }
    ...
    if (simTime == trajectory.back().time)
    {
        leftIndex = trajectory.size() - 1;
        return true;
    }

    leftIndex = 0;
    while (leftIndex < trajectory.size() - 1 && trajectory[leftIndex + 1].time < simTime)
    {
        leftIndex++;
    }

    if (leftIndex < trajectory.size() - 1)
    {
        double gap = trajectory[leftIndex + 1].time - trajectory[leftIndex].time;
        if (gap > TRAJECTORY_GAP_THRESHOLD &&
            simTime > trajectory[leftIndex].time &&
            simTime < trajectory[leftIndex + 1].time)
        {
            return false;
        }
    }
...
double CalculateElevationForUeAtTime(int ueId, const std::string& satName, double simTime)
{
    ...
    const auto& sat = satIt->second;
    if (simTime < sat.trajectory.front().time || simTime > sat.trajectory.back().time)
    {
        return -90.0;
    }

    double satX, satY, satZ, elevUnused, distUnused;
    InterpolatePosition(satName, simTime, satX, satY, satZ, elevUnused, distUnused);

    const auto& ue = g_ues[ueId];
    double dx = satX - ue.ecefX;
    double dy = satY - ue.ecefY;
    double dz = satZ - ue.ecefZ;
    ...
    double ueR = std::sqrt(ue.ecefX * ue.ecefX + ue.ecefY * ue.ecefY + ue.ecefZ * ue.ecefZ);
    double dotRn = dx * (ue.ecefX / ueR) + dy * (ue.ecefY / ueR) + dz * (ue.ecefZ / ueR);
    return std::asin(std::max(-1.0, std::min(1.0, dotRn / distance))) * 180.0 / M_PI;
}

double GetSatelliteRemainingServiceTime(int ueId, const std::string& satName, double simTime)
{
    (void) ueId;
    auto satIt = g_satellites.find(satName);
    if (satIt == g_satellites.end() || satIt->second.trajectory.empty())
    ...
    const auto& sat = satIt->second;
    size_t index = 0;
    if (!FindTrajectoryBracket(sat.trajectory, simTime, index))
    {
        return 0.0;
    }

    size_t segmentEnd = index;
    while (segmentEnd + 1 < sat.trajectory.size())
    {
        double gap = sat.trajectory[segmentEnd + 1].time - sat.trajectory[segmentEnd].time;
        if (gap > TRAJECTORY_GAP_THRESHOLD)
        {
            break;
        }
    ...
    return std::max(0.0, sat.trajectory[segmentEnd].time - simTime);
}
...
UeSatMetrics UpdateSatelliteState(const std::string& satName, double simTime, int ueId)
{
    ...
    if (!FindTrajectoryBracket(it->second.trajectory, simTime, trajectoryIndex))
    {
        return m;
    }
    (void) trajectoryIndex;
...
std::vector<MeasResultCell> UE_PerformMeasurements(int ueId, double simTime)
{
    ...
    for (const auto& pair : ue.channelCache)
    {
        ...
        if (metrics.snrDb < g_minSNR) continue; 
...
{
    if (neighbor.sinr < g_minSNR)
    {
        return false;
    }

    double servingRemaining = GetSatelliteRemainingServiceTime(ueId, serving.cellId, serving.measurementTime);
    double targetRemaining = GetSatelliteRemainingServiceTime(ueId, neighbor.cellId, neighbor.measurementTime);

    return targetRemaining > servingRemaining + reportingOffset;
}

bool UE_EvaluateNtnTriggers(int ueId, const MeasResultCell& serving)
{
    double servingRemaining = GetSatelliteRemainingServiceTime(ueId, serving.cellId, serving.measurementTime);
    return servingRemaining <= g_minServingTime;
}
...
MeasurementReport* UE_EvaluateAndReport(int ueId, double simTime)
{
    ...
    double bestRemainingTime = -1.0;
    double servingRemainingTime = GetSatelliteRemainingServiceTime(ueId, servingMeas->cellId, simTime);
    ...
    for (auto& meas : measurements)
    {
        ...
        double remainingTime = GetSatelliteRemainingServiceTime(ueId, meas.cellId, simTime);
        if (remainingTime > bestRemainingTime ||
            (std::abs(remainingTime - bestRemainingTime) < 1e-9 && meas.distance < bestDist))
        {
            bestNeighbor = &meas;
    ...
    bool betterNeighborAvailable = bestNeighbor && (bestRemainingTime > servingRemainingTime);
    if (bestNeighbor) {
        ...
        std::cout << "[HO_DEBUG]"
                  ...
                  << ",serving_remaining_time=" << std::setprecision(2) << servingRemainingTime
                  << ",neighbor_remaining_time=" << bestRemainingTime
                  << ",serving_trend=" << 0.0
                  << ",neighbor_trend=" << 0.0
        ...
        if (eventA3Triggered)
        {
            ...
            hoContext.triggerType = "SERVING_TIME_A3";
        }
        else if (ntnTrigger && betterNeighborAvailable)
        {
            hoContext.preServingSnr = servingMeas->sinr;
            hoContext.preNeighborSnr = bestNeighbor->sinr;
            hoContext.decisionTime = simTime;
            ...
            hoContext.sourceCellId = servingMeas->cellId;
            hoContext.targetCellId = bestNeighbor->cellId;
            hoContext.triggerType = "SERVING_TIME_CRITICAL";
        }
        else
        {
            ResetHoContext(hoContext);
    ...
    bool triggerConditionMet = eventA3Triggered || (ntnTrigger && betterNeighborAvailable);
        ...
        else
        {
            ...
            if (triggerDuration >= measConfig.timeToTrigger)
            {
                ...
                if (ntnTrigger) {
                    ...
                    report->triggerReason = "Serving satellite remaining time below threshold";
                } else {
                    report->eventType = "CHO_PREP";
                    report->triggerReason = "CHO Phase-1 MR triggered by serving-time advantage";
                }
                
                report->servingCellBelowThreshold = ntnTrigger;
                report->betterNeighborAvailable = betterNeighborAvailable;
...
void CHO_TriggerProcedure(int ueId, const MeasurementReport* report)
{
    ...
    double sourceRemainingTime = GetSatelliteRemainingServiceTime(ueId, choState.sourceCell, currentTime);
    double targetRemainingTime = GetSatelliteRemainingServiceTime(ueId, choState.targetCell, currentTime);
    ...
    std::cout << "  Source: " << choState.sourceCell 
              ...
              << "Dist=" << std::setprecision(0) << sourceDist << " km, "
              << "Remaining=" << std::setprecision(1) << sourceRemainingTime << " s)" << std::endl;
    std::cout << "  Target: " << choState.targetCell 
              << " (SINR=" << std::setprecision(2) << choState.targetSnrAtReport << " dB, "
              << "Dist=" << std::setprecision(0) << targetDist << " km, "
              << "Remaining=" << std::setprecision(1) << targetRemainingTime << " s)" << std::endl;
...
void CHO_CheckExecutionCondition(int ueId, double currentTime)
{
    ...
    // Fetch the current link state from this UE's dedicated channel cache
    ...
    double servingRemainingTime = GetSatelliteRemainingServiceTime(ueId, servingCell, currentTime);
    double targetRemainingTime = GetSatelliteRemainingServiceTime(ueId, targetCell, currentTime);
    double remainingTimeGain = targetRemainingTime - servingRemainingTime;

    bool conditionMet = (servingRemainingTime <= g_minServingTime) &&
                        (remainingTimeGain >= g_choExecutionOffset) &&
                        ((targetIt->second.second.distance / 1000.0) < g_maxDistance) &&
                        (targetIt->second.second.snrDb >= g_minSNR) &&
                        (targetIt->second.second.elevation >= 0.0);
...
// Select best satellite based on remaining service time with hysteresis for a specific UE
std::string SelectBestSatellite(int ueId, const std::string& currentSat)
{
    ...
    // Find current satellite's remaining service time from UE's cache
    double currentRemainingTime = -1.0;
    auto currentIt = ue.channelCache.find(currentSat);
    if (currentIt != ue.channelCache.end())
    {
        currentRemainingTime = GetSatelliteRemainingServiceTime(ueId, currentSat, simTime);
    }
    
    // Find satellite with the longest remaining service time from this UE's perspective
    for (const auto& pair : ue.channelCache)
    {
        ...
        if (metrics.snrDb < g_minSNR)
        {
            continue;
        }

        double remainingTime = GetSatelliteRemainingServiceTime(ueId, satName, simTime);
        if (remainingTime > bestRemainingTime ||
            (std::abs(remainingTime - bestRemainingTime) < 1e-9 && metrics.distance < bestDist))
        {
            bestRemainingTime = remainingTime;
            bestDist = metrics.distance;
            bestSat = satName;
            std::cout << "    → New best candidate: " << bestSat 
                    << " (RemainingTime=" << std::setprecision(1) << bestRemainingTime << " s)" << std::endl;
        }
    }
    
    // Apply serving-time hysteresis: only switch if new satellite can serve meaningfully longer
    if (bestSat != currentSat)

    {
        if (!currentSat.empty() && (bestRemainingTime - currentRemainingTime) < g_choExecutionOffset)
        {
            // New satellite cannot extend service time enough, stay with current
            return currentSat;
        }
    }
    return bestSat;
...
void CheckHandoverConditions()
{
    ...
    // 2. Refresh per-satellite remaining service time
    RefreshSatelliteTiming(simTime);

    // 3. Apply latest physical layer attributes (PER, Delay)
    UpdateChannelState();
    
    // 4. Check if each UE needs to trigger measurement report and handover
    for (int i = 0; i < (int)g_ues.size(); i++)
    {
        CheckHandover(simTime, i);
    }
    
    // 5. Schedule next 1-second check
...
main(int argc, char* argv[])
{
    ...
    std::string ueGroupsCsvFile = "dataset/ue/simulation_groups_taiwan2_suburban.csv";
    ...
    // Serving-time-based handover parameters (PRIMARY)
    cmd.AddValue("reportingOffset", "Minimum target serving-time advantage for reporting (seconds)", g_reportingOffset);
    cmd.AddValue("minSNR", "Minimum SNR threshold (dB)", g_minSNR);
    
    // Fallback and execution parameters
    cmd.AddValue("maxDistance", "Maximum distance threshold for fallback (km)", g_maxDistance);
    cmd.AddValue("minServingTime", "Critical remaining serving time threshold (seconds)", g_minServingTime);
    ...
    cmd.AddValue("choExecutionOffset", "CHO Phase-2 execution serving-time margin (seconds)", g_choExecutionOffset);
    ...
    mkdir("result", 0755);
    std::string outputDir = "result/serving-time-cho";

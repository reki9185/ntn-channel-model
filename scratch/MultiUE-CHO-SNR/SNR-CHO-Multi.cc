...
void CHO_Phase7_ExecutionComplete(int ueId, std::string sourceCell, std::string targetCell)
{
    ...
    double servingSnr = GetCachedSnr(ue, sourceCell);
    double targetSnr = GetCachedSnr(ue, targetCell);
   ...
   g_xnSignalingLog << currentTime << "," << ueId << ","
    ...
    double snrImprovement = postServingSnr - ue.choProcState.servingSnrAtReport;
    ...
    std::cout << "✓ [UE-" << ueId << "] CHO HANDOVER COMPLETE: "
              << sourceCell << " → " << targetCell << std::endl;
...
void CHO_HandleFailure(int ueId, std::string reason, double failureTime)
{
    ...
    double currentServingSnr = GetCachedSnr(ue, ue.choProcState.sourceCell);
    double currentTargetSnr = GetCachedSnr(ue, ue.choProcState.targetCell);
    double decisionDelay = failureTime - ue.choProcState.procedureStartTime;
    double postServingSnr = currentServingSnr;
    double sinrGain = postServingSnr - ue.choProcState.servingSnrAtReport;

    std::cout << "[HO_RESULT],ue=" << ueId
              << ",t=" << std::fixed << std::setprecision(3) << failureTime
              << ",status=FAIL"
              << ",source=" << ue.choProcState.sourceCell
              << ",target=" << ue.choProcState.targetCell
              << ",pre_serving_sinr=" << std::setprecision(2) << ue.choProcState.servingSnrAtReport
              << ",pre_neighbor_sinr=" << ue.choProcState.targetSnrAtReport
              << ",exec_serving_sinr=" << ue.choProcState.servingSnrAtExecution
              << ",exec_target_sinr=" << ue.choProcState.targetSnrAtExecution
              << ",post_serving_sinr=" << postServingSnr
              << ",post_target_sinr=" << currentTargetSnr
              << ",sinr_gain=" << sinrGain
              << ",decision_delay=" << std::setprecision(3) << decisionDelay
              << ",reason=" << reason
              << std::endl;
...
main(int argc, char* argv[])
{
    ...
    mkdir("result", 0755);
    std::string outputDir = "result/snr-cho";

...
// -------------------------------------------------------------------------
// SD-TOPSIS 演算法實作 (基於論文: Evaluation of TOPSIS Algorithm for LEO)
// 準則: 0=SINR (Benefit), 1=Elevation (Benefit), 2=Distance (Cost)
// -------------------------------------------------------------------------
MeasResultCell* FindBestTargetWithSD_TOPSIS(std::vector<MeasResultCell*>& candidates) 
{
    if (candidates.empty()) return nullptr;
    if (candidates.size() == 1) return candidates[0]; // 只有一個候選者直接回傳

    int numCands = candidates.size();
    int numCriteria = 3; 
    
    // 建立決策矩陣 (Decision Matrix)
    std::vector<std::vector<double>> matrix(numCands, std::vector<double>(numCriteria, 0.0));
    for (int i = 0; i < numCands; ++i) {
        matrix[i][0] = candidates[i]->sinr;
        matrix[i][1] = candidates[i]->elevation;
        matrix[i][2] = candidates[i]->distance;
    }

    // 1. 歸一化決策矩陣 (Vector Normalization)
    ...
    std::vector<std::vector<double>> normMatrix = matrix;
    for (int i = 0; i < numCands; ++i) {
        for (int j = 0; j < numCriteria; ++j) {
            if (sumSquares[j] > 0) {
                normMatrix[i][j] /= std::sqrt(sumSquares[j]);
            }
        }
    }

    // 2. SD-TOPSIS 客觀權重計算 (計算標準差)
    std::vector<double> means(numCriteria, 0.0);
    std::vector<double> stdDevs(numCriteria, 0.0);
    double sumStdDev = 0.0;
    
    for (int j = 0; j < numCriteria; ++j) {
        for (int i = 0; i < numCands; ++i) means[j] += normMatrix[i][j];
        means[j] /= numCands;
        
        for (int i = 0; i < numCands; ++i) {
            stdDevs[j] += std::pow(normMatrix[i][j] - means[j], 2);
        }
        stdDevs[j] = std::sqrt(stdDevs[j] / numCands);
        sumStdDev += stdDevs[j];
    }

    // 賦予權重 (若變異度為0則給予平均權重)
    std::vector<double> weights(numCriteria, 1.0 / numCriteria);
    if (sumStdDev > 1e-6) {
        for (int j = 0; j < numCriteria; ++j) weights[j] = stdDevs[j] / sumStdDev;
    }

    // 3. 計算加權歸一化矩陣
    for (int i = 0; i < numCands; ++i) {
        for (int j = 0; j < numCriteria; ++j) normMatrix[i][j] *= weights[j];
    }

    // 4. 定義正理想解 (PIS) 與負理想解 (NIS)
    // index 0: SINR (越大越好), index 1: Elev (越大越好), index 2: Dist (越小越好)
    std::vector<double> pis = { -1e9, -1e9, 1e9 }; 
    std::vector<double> nis = { 1e9, 1e9, -1e9 };
    
    for (int i = 0; i < numCands; ++i) {
        pis[0] = std::max(pis[0], normMatrix[i][0]); nis[0] = std::min(nis[0], normMatrix[i][0]);
        pis[1] = std::max(pis[1], normMatrix[i][1]); nis[1] = std::min(nis[1], normMatrix[i][1]);
        pis[2] = std::min(pis[2], normMatrix[i][2]); nis[2] = std::max(nis[2], normMatrix[i][2]);
    }

    // 5. 計算各衛星與 PIS/NIS 的歐式距離，並得出最終分數
    int bestIndex = 0;
    double highestScore = -1.0;

    // 將 TOPSIS 分數記錄下來方便 DEBUG (可選)
    std::vector<double> scores(numCands, 0.0);

    for (int i = 0; i < numCands; ++i) {
        double distPIS = 0.0, distNIS = 0.0;
        for (int j = 0; j < numCriteria; ++j) {
            distPIS += std::pow(normMatrix[i][j] - pis[j], 2);
            distNIS += std::pow(normMatrix[i][j] - nis[j], 2);
        }
        distPIS = std::sqrt(distPIS);
        distNIS = std::sqrt(distNIS);
        
        double score = (distPIS + distNIS > 0) ? (distNIS / (distPIS + distNIS)) : 0.0;
        scores[i] = score;
...
{
    auto& measState = g_ues[ueId].measState;
    auto& measConfig = g_ues[ueId].measConfig;

    if (measState.servingCellId.empty()) return nullptr;
    ...
    std::vector<MeasResultCell*> validCandidates;
    ...
    for (auto& meas : measurements)
    {
        ...
        // 將符合條件的候選衛星加入陣列
        validCandidates.push_back(&meas);
    }
    
    // 🔥 核心修改：使用 SD-TOPSIS 演算法選出綜合表現最佳的目標衛星
    MeasResultCell* bestNeighbor = FindBestTargetWithSD_TOPSIS(validCandidates);
...
main(int argc, char* argv[])
{
    ...
    mkdir("result", 0755);
    std::string outputDir = "result/sd-topsis-cho";
